...
G_MODULE_EXPORT void on_menu_change_lang_button_press_event(GtkWidget *window, gpointer data)
{
    ...
    if (lang_code)
    {
        ...
        gtk_label_set_text(mainWindowObjects.lbl_avg_daily_temp, "RAverage Daily Air Temperature, °C");
...
{
    struct ps *pss = (struct ps *)malloc(sizeof(struct ps));
    GtkTextIter start, end;
...
G_MODULE_EXPORT void on_menu_about_button_press_event(GtkWidget *window, gpointer data)
{
    if (lang_code)
    {
        GtkWidget *dialog = gtk_dialog_new_with_buttons(
            "About", mainWindowObjects.main_window,
            GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, "_OK", GTK_RESPONSE_NONE, NULL);
        GtkWidget *content_area = gtk_dialog_get_content_area(GTK_DIALOG(dialog));
        gtk_container_set_border_width(GTK_CONTAINER(content_area), 15);
        GtkWidget *label = gtk_label_new("\nThe software is developed by Mikhail Zimin according to РД 52.37.612-2000 in 2002\ne-mail: zimin7@yandex.ru\n");
        gtk_container_add(GTK_CONTAINER(content_area), label);
        gtk_widget_show(label);
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
    ...
    else
    {
        ...
        GtkWidget *content_area = gtk_dialog_get_content_area(GTK_DIALOG(dialog));
        gtk_container_set_border_width(GTK_CONTAINER(content_area), 15);
        GtkWidget *label = gtk_label_new("\nПрограмма разработана М. И. Зиминым в соответствии с РД 52.37.612-2000 в 2002 г.\ne-mail: zimin7@yandex.ru\n");
        gtk_container_add(GTK_CONTAINER(content_area), label);
        gtk_widget_show(label);
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
